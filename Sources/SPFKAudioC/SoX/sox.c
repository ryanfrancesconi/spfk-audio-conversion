...
} combine_method = sox_merge;//sox_default;
...
static struct {
    char *name; int argc; char **argv; size_t argv_size;
} **user_effargs = NULL;
...
/* Cleanup atexit_cleanup() function, hence always called. */
static void atexit_cleanup(void) {
    /* Do not call cleanup using atsox_error() if possible.  pthread's can
...
static char const * str_time(double seconds) {
    ...
    mins -= hours * 60;
    i = (i + 1) & 15;
    // snprintf(string[i], "%02i:%02i:%05.2f", hours, mins, seconds);
    return string[i];
}
...
static void play_file_info(sox_format_t *ft, file_t *f, sox_bool full) {
    ...
    snprintf(buffer, "  Channels: %u @ %u-bit", ft->signal.channels, ft->signal.precision);
    ...
    //snprintf(buffer, "Samplerate: %g Hz", ft->signal.rate);
    fprintf(output, "%-25s", buffer);
    ...
    if (f && f->replay_gain != HUGE_VAL) {
        snprintf(buffer, "%s gain: %+.1fdB", lsx_find_enum_value(f->replay_gain_mode, rg_modes)->text, f->replay_gain);
        buffer[0] += 'A' - 'a';
...
static void display_file_info(sox_format_t *ft, file_t *f, sox_bool full) {
    static char const *const no_yes[] = {
        "no", "yes"
    };
    ...
    if (ft->encoding.encoding) {
        ...
        // if (ft->encoding.bits_per_sample) snprintf(buffer, "%u-bit ", ft->encoding.bits_per_sample);
...
static void auto_effect(sox_effects_chain_t *chain, char const *name, int argc,
    ...
    if (sox_effect_options(effp, argc, argv) == SOX_EOF) {
        sox_error(1);                            /* The failing effect should have displayed an error message */
        return;
    }

    if (add_effect(chain, effp, signal, &ofile->ft->signal, guard) != SOX_SUCCESS) {
        sox_error(2);  /* The effects chain should have displayed an error message */
        return;
    }
...
static void parse_effects(int argc, char **argv) {
    while (optstate.ind < argc) {
        ...
        user_effargs[eff_chain_count][eff_offset].argc = (int)j;
...
static void read_user_effects(char const *filename) {
    ...
    while (fgets(s + pos, (int)(buffer_size - pos), file)) {
        int len = (int)strlen(s + pos);

        if (len && s[pos + len - 1] == '\n') {
            s[pos + len - 1] = '\0';
            pos = 0; /* we've read a complete line */
...
static void add_effects(sox_effects_chain_t *chain) {
    ...
    for (i = 0; i < nuser_effects[current_eff_chain] &&
        ...
        if (add_effect(chain, user_efftab[i], &signal, &ofile->ft->signal,
                       &guard) != SOX_SUCCESS) {
            sox_error(2); /* Effects chain should have displayed an error message */
            return;
        }
...
static char * headroom(void) {
    if (min_headroom < MIN_HEADROOM) {
        ...
        snprintf(buff, "Hd:%u.%u", h / 10, h % 10);
        return buff;
    }
...
#else  /* ifdef HAVE_SOUNDCARD_H */
static void adjust_volume(int delta) {
    (void)delta;
    putc('\a', stderr);
}

#endif /* ifdef HAVE_SOUNDCARD_H */
...
static void calculate_combiner_signal_parameters(void) {
    ...
    if (combine_method == sox_sequence) {
        ...
        combiner_signal.channels =
            combine_method == sox_merge ? (int)total_channels : (int)max_channels;
...
#define SOX_BUFMIN 16
                    ...
                    case 13: display_SoX_version(stdout); sox_error(0); break;
...
static int soxi(int argc, char *const *argv) {
    ...
    while ((opt = lsx_getopt(&optstate)) > 0) /* act only on last option */
        if (opt == 'V') {
            ...
            do_total = sox_true;
        } else if ( (type = 1 + (soxi_t)(strchr(opts, opt) - opts)) > Annotation) {
            soxi_usage(1);
        }
...
// was main
int sox_main_private(int argc, char *argv[]) {
    size_t i;
    char mybase[6];
...
//            (files[j]->ft->handler.flags & SOX_FILE_PHONY) == 0) show_progress = sox_option_yes;
    }
    ...
    for (i = 0; i < file_count; ++i) {
        if (files[i]->ft->clips != 0) {
            lsx_warn(i < input_count ? "`%s' input clipped %i samples" :
                     "`%s' output clipped %i samples; decrease volume?",

                     (files[i]->ft->handler.flags & SOX_FILE_DEVICE) ?
                     files[i]->ft->handler.names[0] : files[i]->ft->filename,
                     (int)files[i]->ft->clips);
        }
    }
...
int sox_main(int argc, char *argv[]) {
    int status = sox_main_private(argc, argv);

    cleanup_globals();
    return status;
}

void sox_error(int code) {
    fprintf(stderr, "sox_error code: %u\n", code);
}
