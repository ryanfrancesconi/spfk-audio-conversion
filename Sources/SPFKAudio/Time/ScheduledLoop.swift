...
public struct ScheduledLoop {
    ...
    public var hostTime: UInt64 = mach_absolute_time()
    ...
    public var firstDuration: TimeInterval?
    ...
    public mutating func next() -> AVAudioTime? {
        ...
        if isInfinite, times.count == 1 {
            Log.debug("⚠️ Need to generate more loops")
            appendSchedule()
        }
    ...
    public mutating func createSchedule(firstDuration: TimeInterval? = nil,
        ...
        // represents a partial first loop
        if let firstDuration = firstDuration,
           firstDuration > 0,
